...
import { DollarSign, Calendar, FileText, AlertCircle, CheckCircle, Trash2, Clock, TrendingUp, CreditCard, Pencil } from 'lucide-react';
...
interface RecentRevenue extends Revenue {
  units?: { name: string; color: string };
  categories?: { name: string };
  subcategories?: { name: string };
}

export default function LaunchRevenuePage() {
  const { user } = useAuth();
  const [recent, setRecent] = useState<RecentRevenue[]>([]);
  ...
  const [editingRecord, setEditingRecord] = useState<RecentRevenue | null>(null);
  ...
  async function fetchRecent() {
    const { data } = await supabase
      ...
      .limit(10);
    if (data) setRecent(data as RecentRevenue[]);
  }
  ...
  async function handleSubmit() {
    ...
    if (!form.date) { setError('Informe a data da receita.'); return; }
    ...
    setSaving(false);
    if (err) { setError('Erro ao lançar receita. Tente novamente.'); return; }
    ...
    <div className="max-w-3xl mx-auto px-4 py-8">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-1">
          ...
          <h2 className="text-2xl font-bold text-white">Lançar Receita</h2>
        </div>
        <p className="text-slate-400 text-sm ml-11">Registre uma nova entrada financeira</p>
      </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 mb-8">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                placeholder="0,00"
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/60 border border-slate-700 focus:border-emerald-500/50 text-white placeholder-slate-500 rounded-xl text-sm outline-none transition-colors"
              />
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/60 border border-slate-700 focus:border-emerald-500/50 text-white rounded-xl text-sm outline-none transition-colors"
              />
            </div>
          </div>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="relative">
              ...
              <select
                ...
                className="w-full appearance-none pl-10 pr-4 py-2.5 bg-slate-800/60 border border-slate-700 focus:border-emerald-500/50 text-white rounded-xl text-sm outline-none transition-colors cursor-pointer"
              >
          ...
          <div className="sm:col-span-2">
            ...
            <div className="relative">
              ...
              <textarea
                ...
                rows={3}
                className="w-full pl-10 pr-4 py-2.5 bg-slate-800/60 border border-slate-700 focus:border-emerald-500/50 text-white placeholder-slate-500 rounded-xl text-sm outline-none transition-colors resize-none"
              />
            </div>
          </div>
        </div>
        ...
        <button
          ...
          disabled={saving}
          className="w-full mt-5 py-3.5 bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-slate-950 font-bold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/15 hover:shadow-emerald-500/30"
        >
          {saving ? 'Registrando...' : 'Lançar Receita'}
        </button>
      </div>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  <div
                    ...
                    style={{ backgroundColor: (revenue.units?.color || '#00FF88') + '22' }}
                  >
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: revenue.units?.color || '#00FF88' }} />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-sm font-semibold text-white">{revenue.units?.name || '—'}</span>
                      {revenue.categories?.name && (
                        <>
                          <span className="text-slate-600">/</span>
                          <span className="text-xs text-slate-400">{revenue.categories.name}</span>
                        </>
                      )}
                      {revenue.subcategories?.name && (
                        <>
                          <span className="text-slate-600">/</span>
                          <span className="text-xs text-slate-500">{revenue.subcategories.name}</span>
                        </>
                      )}
                    ...
                    <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                      ...
                      {revenue.observation && (
                        <p className="text-xs text-slate-500 truncate max-w-xs">{revenue.observation}</p>
                      )}
                    </div>
                  </div>
                </div>
                <div className="flex items-center gap-2 ml-3 flex-shrink-0">
                  ...
                  <button
                    onClick={() => setEditingRecord(revenue)}
